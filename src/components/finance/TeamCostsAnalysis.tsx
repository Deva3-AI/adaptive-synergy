      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="flex flex-col p-4 bg-muted/50 rounded-lg">
            ...
            <span className="text-2xl font-bold">{formatCurrency(data.total_cost)}</span>
            <span className="text-xs text-muted-foreground">{period}</span>
          </div>
          
          <div className="flex flex-col p-4 bg-muted/50 rounded-lg">
            <span className="text-sm text-muted-foreground">Average per Employee</span>
            <span className="text-2xl font-bold">{formatCurrency(data.average_cost_per_employee)}</span>
            <span className="text-xs text-muted-foreground">{period}</span>
          </div>
          
          <div className="flex flex-col p-4 bg-muted/50 rounded-lg">
            <span className="text-sm text-muted-foreground">Cost to Revenue</span>
            <span className="text-2xl font-bold">{(data.efficiency_metrics.cost_to_revenue_ratio * 100).toFixed(1)}%</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            ...
            <BarChart 
              data={data.cost_by_department.map(dept => ({
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-muted/50 p-4 rounded-lg flex flex-col">
            ...
            <div className="text-xl font-semibold">{formatCurrency(data.efficiency_metrics.revenue_per_employee)}</div>
            <div className="text-xs text-muted-foreground">{period}</div>
          </div>
          
          <div className="bg-muted/50 p-4 rounded-lg flex flex-col">
            <div className="text-sm text-muted-foreground">Profit per Employee</div>
            <div className="text-xl font-semibold">{formatCurrency(data.efficiency_metrics.profit_per_employee)}</div>
            <div className="text-xs text-muted-foreground">{period}</div>
          </div>
          
          <div className="bg-muted/50 p-4 rounded-lg flex flex-col">
            <div className="text-sm text-muted-foreground">Cost to Revenue</div>
            <div className="text-xl font-semibold">{(data.efficiency_metrics.cost_to_revenue_ratio * 100).toFixed(1)}%</div>
            <div className="text-xs text-muted-foreground">{period}</div>
          </div>
        </div>
      </CardContent>
